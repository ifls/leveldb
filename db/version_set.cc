...
namespace leveldb {
	...
	// 搜索sst table
	void Version::ForEachOverlapping(Slice user_key, Slice internal_key, void *arg, bool (*func)(void *, int
			, FileMetaData *)) {
		...
		if (!tmp.empty()) {
			...
			for (uint32_t i = 0; i < tmp.size(); i++) {
				if (!(*func)(arg, 0, tmp[i])) {  //func 判断key是否在此文件内，并拿到值
					return;
				}
			}
		}
		...
		for (int level = 1; level < config::kNumLevels; level++) {
			...
			// 二分查找是否在当前层有此数据 Binary search to find earliest index whose largest key >= internal_key.
			...
			if (index < num_files) {
				...
				if (ucmp->Compare(user_key, f->smallest.user_key()) < 0) {
					...
					if (!(*func)(arg, level, f)) {  //func 判断key是否在此文件内，并拿到值
						return;
					}
				}
			}
	...
	public:
		...
		// 应用sst文件修改，记录 到level_数组中
		void Apply(VersionEdit *edit) {  //class Builder
			// Update compaction pointers
			for (size_t i = 0; i < edit->compact_pointers_.size(); i++) {
				const int level = edit->compact_pointers_[i].first; //key
		...
		// 生成新的version
		void SaveTo(Version *v) {  //class Builder
			BySmallestKey cmp;
			cmp.internal_comparator = &vset_->icmp_;
			for (int level = 0; level < config::kNumLevels; level++) {
				...
				v->files_[level].reserve(base_files.size() + added_files->size()); // 预留内存
	...
	VersionSet::VersionSet(const std::string &dbname, const Options *options, TableCache *table_cache
			...
			: env_(options->env), dbname_(dbname), options_(options), table_cache_(table_cache), icmp_(*cmp), next_file_number_(2), manifest_file_number_(0),  // Filled by Recover()
			  last_sequence_(0), log_number_(0), prev_log_number_(0), descriptor_file_(nullptr), descriptor_log_(nullptr), dummy_versions_(this), current_(nullptr) {
	...
	Status VersionSet::LogAndApply(VersionEdit *edit, port::Mutex *mu) {
		...
		Version *v = new Version(this);   // 新建version
		{
			...
			builder.Apply(edit);  //version + edit 保存edit信息
			builder.SaveTo(v);   // 算出新的version new_version =
		}
		...
		if (descriptor_log_ == nullptr) {
			...
			edit->SetNextFile(next_file_number_);
			s = env_->NewWritableFile(new_manifest_file, &descriptor_file_);  // 第二个参数是返回结果
			if (s.ok()) {
	...
	Status VersionSet::Recover(bool *save_manifest) {
		...
		SequentialFile *file;
		s = env_->NewSequentialFile(dscname, &file); // 打开当前的manifest文件
		if (!s.ok()) {
			if (s.IsNotFound()) {
		...
		Builder builder(this, current_);  // current_ 是当前版本

		{
			...
			Slice record;
			std::string scratch;
			while (reader.ReadRecord(&record, &scratch) && s.ok()) {  // 一条一条读出versionEdit
				VersionEdit edit;
		...
		if (s.ok()) {
			...
			builder.SaveTo(v);  // 上面多次apply, 得到最终的version
			// Install recovered version
			Finalize(v);

			AppendVersion(v); // 加入version链表里
	...
	// 返回是否重用旧的 manifest文件,而不是每次启动都生成新的, 提高启动效率
	...
	// 计算 compact_level 和 compact_score
	...
	const char *VersionSet::LevelSummary(LevelSummaryStorage *scratch) const {
		...
		std::snprintf(scratch->buffer, sizeof(scratch->buffer), "files[ %d %d %d %d %d %d %d ]", int(current_->files_[0].size()), int(current_->files_[1].size()), int(current_->files_[2].size()), int(current_->files_[3].size()), int(current_->files_[4].size()), int(current_->files_[5].size()), int(current_->files_[6].size()));
	...
	void VersionSet::AddLiveFiles(std::set<uint64_t> *live) {
		for (Version *v = dummy_versions_.next_; v != &dummy_versions_; v = v->next_) {  //所有版本
			for (int level = 0; level < config::kNumLevels; level++) {  //所有level
				const std::vector<FileMetaData *> &files = v->files_[level];
				for (size_t i = 0; i < files.size(); i++) {  // 每层所有文件
					live->insert(files[i]->number);  //保存 文件 number
				}
			}
		}
	}
		...
		if (!c->inputs_[1].empty()) { // l(n+1) 有文件可以合并
			...
			if (expanded0.size() > c->inputs_[0].size() &&
				...
				if (expanded1.size() == c->inputs_[1].size()) {
					Log(options_->info_log, "Expanding@%d %d+%d (%ld+%ld bytes) to %d+%d (%ld+%ld bytes)\n", level, int(c->inputs_[0].size()), int(c->inputs_[1].size()), long(inputs0_size), long(inputs1_size), int(expanded0.size()), int(expanded1.size()), long(expanded0_size), long(inputs1_size));
					smallest = new_start;
					largest = new_limit;
	...
	Compaction::Compaction(const Options *options, int level)
			: level_(level), max_output_file_size_(MaxFileSizeForLevel(options, level)), input_version_(nullptr), grandparent_index_(0), seen_key_(false), overlapped_bytes_(0) {
