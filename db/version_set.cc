	...
	struct State {
	  ...
	  static bool Match(void *arg, int level, FileMetaData *f) {
		  ...
		  switch (state->saver.state) {
			  case kNotFound: return true;  // Keep searching in other files
			  case kFound: state->found = true;
				  return false;
			  case kDeleted: return false;
			  case kCorrupt: state->s = Status::Corruption("corrupted key for ", state->saver.user_key);
				  state->found = true;
				  return false;
		  }
	...
	for (int level = 0; level < config::kNumLevels; level++) {
		...
		for (size_t i = 0; i < files.size(); i++) {
			if (icmp_.Compare(files[i]->largest, ikey) <= 0) {
				...
				Table *tableptr;
				Iterator
					*iter = table_cache_->NewIterator(ReadOptions(), files[i]->number, files[i]->file_size, &tableptr);
				if (tableptr != nullptr) {
