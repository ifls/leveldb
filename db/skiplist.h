...
// Apart from that 除此外, reads progress without any internal locking or synchronization.
//
// Invariants: 不变性
//
...
class SkipList {
 ...
 public:
  ...
  // Insert key into the list. 不能插入重复key
 ...
 private:
  ...
  Node *const head_;    // 链表头指针

  // Modified only by Insert().  Read racily by readers, but stale
  // values are ok.
  std::atomic<int> max_height_;  // Height of the entire list

  // Read/written only by Insert().  rnd_.Next()
  Random rnd_;
};
