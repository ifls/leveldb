...
class Repairer {
 public:
  Repairer(const std::string &dbname, const Options &options)
	  : dbname_(dbname),
		...
		owns_info_log_(
			options_.info_log != options.info_log),
		owns_cache_(
			options_.block_cache != options.block_cache),
		next_file_number_(1) {
  ...
  // 修复db 即使 manifest 文件损坏或者丢失
  Status Run() {
	  Status status = FindFiles();
	  if (status.ok()) {
		  ConvertLogFilesToTables();  // 将log 整理成一个sst
		  ExtractMetaData();  //扫描所有sst 生成 元数据
		  status = WriteDescriptor();  // 写入 生成manifest文件
	  }
	  if (status.ok()) {
 ...
 private:
  ...
  Status WriteDescriptor() {
	  ...
	  WritableFile *file;
	  Status status = env_->NewWritableFile(tmp, &file); // 准备新的manifest文件
	  if (!status.ok()) {
		  return status;
	  }
	  ...
	  for (size_t i = 0; i < tables_.size(); i++) {
		  ...
		  const TableInfo &t = tables_[i];   // 暂时所有的都放在0层, 等待之后的合并, 效率低
