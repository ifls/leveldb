...
class Benchmark {
 ...
 public:
  ...
  Benchmark()
	  : db_(nullptr), num_(FLAGS_num), reads_(FLAGS_reads < 0 ? FLAGS_num : FLAGS_reads), bytes_(0), rand_(301) {
 ...
 private:
  void Open(bool sync) {
	  ...
	  std::string test_dir;
	  Env::Default()->GetTestDirectory(&test_dir);
	  std::snprintf(file_name, sizeof(file_name), "%s/dbbench_polyDB-%d.kct", test_dir.c_str(), db_num_);
